...
def prepare_prediction_data(df, lookback=5):
    ...
    for i in range(lookback, len(df) - 4):
        ...
        volume_change_history = volume_change.iloc[i-lookback:i].values

        features = np.concatenate([close_prices, log_return_history])
        features = np.concatenate([close_prices, log_return_history, volume_change_history])
        X.append(features)
    ...
    def next(self):
        if len(self.data) >= 6:
            try:
                ...
                last_volume_change.reverse()
                last_features = last_close_prices + last_log_returns + last_volume_change
